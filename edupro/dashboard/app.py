...
from src import (
    generate_data, load_raw_data, engineer_features,
...
def load_all():
    # Prefer real EdUPro CSVs generated by data/raw/generate.py;
    # fall back to generate_data() if the files don't exist yet.
    try:
        users, courses, transactions = load_raw_data(
            'data/raw' if __import__('os').path.exists('data/raw/users.csv')
            else '../data/raw'
        )
    except FileNotFoundError:
        users, courses, transactions = generate_data(n_users=500, n_courses=200)

    # Normalise date column: EdUPro v2 uses 'EnrollmentDate', legacy uses 'TransactionDate'.
    # We rename to 'TransactionDate' so all downstream dashboard code stays consistent.
    if 'EnrollmentDate' in transactions.columns and 'TransactionDate' not in transactions.columns:
        transactions = transactions.rename(columns={'EnrollmentDate': 'TransactionDate'})
